...
原创 挖土机信息学奥赛 挖土机青少年编程 2024-04-18 17:02 陕西

转载 Eggest 2024-4-18 18:58 陕西

[语法周赛 Round 12] 的题目解析与赛后分析如下，还没有完成的可以登录 OJ 补题 [https://oj.since2018.top/]

1 语法周赛 Round 12 试题分析
...
参考代码：此处还是按照个人的习惯，对于时间类的问题转换为最小单位后在处理。显然在本题中，“转换为分钟”、“往前数 15 小时”、“转换回小时” 可以直接用 a -= 15; if (a < 0) a += 24; 代替。
```cpp
#include <bits/stdc++.h>
using namespace std;
int main()
...
子任务 1（30 分）：当 n 为一位数时，直接输入 1∼n 之和即可。

子任务 2（30 分）：数据范围很小，直接用 i 暴力枚举 1∼n 之内的每个整数，使用题目给出的“求数位和”的代码求出“一位数位和”然后求和即可。

子任务 3（40 分）：容易发现当数据范围很大时上面的思路会超时，可以考虑先打印出每个数的一位数位和
看看规律，很容易发现就是 1∼9 的循环，直接根据数学规律算出答案即可。注意要用 long long。
...
子任务 2（30 分）：根据题目描述，容易发现 n 为 1∼12 时答案为 0，为 13 时答案为 1，为 14∼29 

时答案都为 2，30∼33 时答案为 3。这题的 60 分就是一个条件判断的题目。
...
```
2 语法周赛 Round 12 成绩分析

参赛学员人数：87 人

参赛学员平均分：217.39 分（满分 400 分）

总分中位数：190 分（满分 400 分）
